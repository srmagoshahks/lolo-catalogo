/**
 * LOLO SOBRE RUEDAS — Chat Widget v5.0 (Con Panel Secreto de Admin + Voz Clonada + Micrófono STT)
 */

const LOLo_CONFIG = {
  API_BASE: (window.location.origin.includes('http') && !window.location.origin.startsWith('file')) ? window.location.origin : 'http://127.0.0.1:5005',
...
(function() {
  ...
  function initSpeechRecognition() {
    ...
    recognition.onstart = function() {
      ...
      inputField.placeholder = '🎙️ Escuchando... Hablá ahora';
    };
  ...
  function stopListening() {
    ...
    inputField.placeholder = 'Escribí tu consulta...';
  }
  ...
  // --- TRIGGER SECRETO DE ADMIN (3 CLICS EN EL LOGO DE LA PÁGINA) ---
  function initSecretAdminTrigger() {
    ...
    // Delegación global por si se genera dinámicamente
  ...
  function buildChatHTML() {
    const headerLogo = `<img class="header-logo" src="${LOLo_CONFIG.LOGO_URL}" alt="Lolo" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="header-logo-fallback" style="display:none">🛹</div>`;
